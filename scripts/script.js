...
const options = {
  ...
  headers: {
    ...
    Authorization:
      "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI5Y2NmZTQzNDMwOTMyZWIzZGZhNDMwZjJjMzkxMDcyMiIsInN1YiI6IjY1YTMwZmY4ZTljMGRjMDEyMGE0NDMxNyIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.rYRQ0qRL2zNllfW0uyAaviSDnssgYtDCEjz72v1o75Y",
  },
};
...
fetch(
  ...
  .then((data) => {
    ...
    data.results.forEach((value, index, array) => {
      ...
      card.className =
        "card card-clickable col-lg-3 col-md-4 col-sm-6 my-3 mx-auto";
...
function openMovieModal(movie) {
  ...
  // Populate modal
  modalTitle.textContent = `${movie.title} (${new Date(
    movie.release_date
  ).getFullYear()})`;
  modalPoster.src = `${baseImageUrl}${movie.poster_path}`;
  modalReleaseDate.textContent = movie.release_date;
  modalRuntime.textContent = `${Math.floor(movie.runtime / 60)}h ${
    movie.runtime % 60
  }min`;
